...
export default function MobileTopBar({ nav }: { nav: NavLink[] }) {
  return (
    <header className="absolute inset-x-0 top-0 z-10 md:hidden" data-node-id="375:1739">
      {/* margin top zmniejszony o połowę (44→22, decyzja Patryka); burger
          trzyma pion ze środkiem logo (22+35 = 37+20 = 57) */}
      <Link href="/" className="absolute left-[15px] top-[22px] h-[70px] w-[61px]">
        <img src="/assets/m-logo-f.png" alt="Kompetencje.ai" className="h-full w-full" />
      </Link>
      <MobileMenu nav={nav} className="absolute right-[20px] top-[37px] h-[40px] w-[40px]" />
    </header>
  );
}
